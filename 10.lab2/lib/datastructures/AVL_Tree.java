...
/**
 * A binary searchtree ordered by the method compareTo for the elements and
 * perform height balancing when adding or removing elements
 * 
 * @author (Bror Bjerner)
 * @version (2007)
 */

public class AVL_Tree<E extends Comparable<? super E>> extends
		BinarySearchTree<E> {
	...
	private class AVL_Entry extends Entry {
		...
		private AVL_Entry(E element, Entry parent) {
	...
	private void checkHeight(Entry t) {
		((AVL_Entry) t).height = 1 + Math.max(height(t.left), height(t.right));
	} // CheckHeight
	...
	/**
	 * Add the element to its first proper empty place. After the element is
	 * inserted the height balance is checked and if nescessary restored.
	 * 
	 * @param elem
	...
	/**
	 * Remove the first occurance of an element with the same key as the
	 * argument element. If no element is removed false is returned, otherwise
	 * true is returned. After the element is removed the height balance is
	 * checked and if nescessary restored.
	 * 
	 * @param elem
	 *            any element with the searched key
	 * @return true if an element has disapeared from the tree, false otherwise
	 */
	...
	/*
	 * Rotera 1 steg i högervarv, dvs x' y' / \ / \ y' C --> A x' / \ / \ A B B
	 * C
	 */
	...
	/*
	 * Rotera 1 steg i vänstervarv, dvs x' y' / \ / \ A y' --> x' C / \ / \ B C
	 * A B
	 */
	...
	/*
	 * Rotera 2 steg i högervarv, dvs x' z' / \ / \ y' D --> y' x' / \ / \ / \ A
	 * z' A B C D / \ B C
	 */
	...
	/*
	 * Rotera 2 steg i vänstervarv, dvs x' z' / \ / \ A y' --> x' y' / \ / \ / \
	 * z D A B C D / \ B C
	 */
